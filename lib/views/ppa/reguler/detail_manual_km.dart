...
import 'package:lmb_online/services/operasi/reguler_perkotaan/delete_lmb_manual_km.dart';
import 'package:lmb_online/services/operasi/reguler_perkotaan/get_lmb_manual_km.dart';
import 'package:lmb_online/services/operasi/reguler_perkotaan/post_lmb_manual_km.dart';
...
class DetailManualKm extends StatefulWidget {
  final GetLmbAdminData lmbData;

  const DetailManualKm({Key? key, required this.lmbData}) : super(key: key);

  @override
  State<DetailManualKm> createState() => _DetailManualKmState();
}

class _DetailManualKmState extends State<DetailManualKm> {
  final GlobalKey<FormState> _formKeyKmOperasional = GlobalKey<FormState>();
  ...
  Future<void> _handleDeleteLmbManualKm(String id_lmb_reguler) async {
    ...
    try {
      var response = await deleteLmbManualKm.deleteLmbManualKm(
        id_lmb!,
        id_lmb_reguler_value!,
        cuser!,
        token!,
      );

      if (response.code == 201) {
        _listLmbManualKmData.removeWhere(
          (item) => item.id_lmb_reguler == id_lmb_reguler_value,
        );
        Navigator.pop(context);
        _dialogSuccess(
          header: "Delete LMB Manual KM",
      ...
      _dialogFailed('Terjadi kesalahan saat menghapus data LMB Manual KM', 500);
    }
  }

  Future<void> _handlePostLmbManualKm() async {
    final prefs = await SharedPreferences.getInstance();
    ...
    final String? km_ritase = _kmOperasionalValue.toString();
    final String? cuser = prefs.getString('user_id');
    final String? kode_layanan = widget.lmbData.kode_layanan;

    print('POST LMB MANUAL KM ID_LMB : $id_lmb');
    print('POST LMB MANUAL KM ID_trayek : $id_trayek');
    print('POST LMB MANUAL KM ID_trayek_detail : $id_trayek_detail');
    print('POST LMB MANUAL KM ritase : $ritase');
    print('POST LMB MANUAL KM km_ritase : $km_ritase');
    print('POST LMB MANUAL KM kode_layanan : $kode_layanan');
    print('POST LMB MANUAL KM waktu : $waktu');
    print('POST LMB MANUAL KM cuser : $cuser');

    try {
      var response = await postLmbManualKm.postLmbManualKm(
        id_lmb!,
        id_trayek!,
        id_trayek_detail!,
        ritase!,
      ...
      print('response postLmbManualKm : $response');

      if (response.code == 201) {
        Navigator.pop(context);
        _dialogSuccess(
          header: 'Berhasil',
          tittle: 'Data LMB Manual KM Berhasil Ditambahkan',
        );
      } else {
        Navigator.pop(context);
        _dialogFailed(response.message, response.code);
  ...
  Future<void> _handleGetLmbManualKm() async {
    final prefs = await SharedPreferences.getInstance();
    final String? token = prefs.getString('token');
    final String? id_lmb = widget.lmbData.id_lmb;

    try {
      var response = await getLmbManualKm.getLmbManualKm(id_lmb!, token!);

      if (response.code == 200 && response.data != null) {
        setState(() {
          _listLmbManualKmData = response.data!;
        });
        print("Data REGULER: $_listLmbManualKmData");
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("${response.message}"),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text(
            "Terjadi kesalahan saat mengambil data LMB manual km",
          ),
          backgroundColor: Colors.red,
        ),
      );
      ...
      child: GestureDetector(
        onTap: () {
          setState(() {
            ...
            _kmOperasionalController.text = _selectedKmBaku;
            print('KM BAKU: $_selectedKmBaku');
          });
          Navigator.pop(context);
          _initData();
        },
                                ...
                                Row(
                                  ...
                                  children: [
                                    ...
                                    Container(
                                      ...
                                      child: ElevatedButton(
                                        onPressed: () async {
                                          await _handleDeleteLmbManualKm(
                                            id_lmb_reguler,
                                          );
                                        },
                                        style: ElevatedButton.styleFrom(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Column(
                        children:
                            _listLmbManualKmData.isNotEmpty
                                ? _listLmbManualKmData.map((item) {
                                  return Padding(
                                    padding: const EdgeInsets.symmetric(
                                      vertical: 5,
                                    ),
